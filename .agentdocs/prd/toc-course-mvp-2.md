...
- [ ] 明确 2.0 不改 1.0 的生成核心，只新增产品化输入、记忆、编辑和续作层。
- [ ] 定义 `IntakeSession`、`StudentPreferenceProfile`、`LearningState`、`GuardianPolicy` 的最小字段。
- [ ] 定义数据最小化规则：不保存学生真实姓名、头像、语音和精确画像；偏好与学习状态按 profileId 关联。
- [ ] 输出 `agent-test/docs/product/mvp2-intake-flow.md`。

验证：

- [ ] schema 单测覆盖缺失年级、缺失学科、上传图片被禁用、时长超限。
- [ ] 隐私规则单测确保敏感字段不会进入持久化模型。
...
- [ ] `packages/core/src/course/product/intakeSession.test.ts`

实现要求：

- [ ] 输入可以是自然语言、部分 `CourseSpec`、历史偏好和家长限制。
- [ ] 输出 `IntakeSession`，明确 `knownFields`、`missingFields`、`followUpQuestions`、`confidence`。
- [ ] 只有缺少高影响字段时才追问；低影响字段使用默认值并记录 assumption。
- [ ] `status === ready_for_plan` 时必须能转换成 1.0 的 `CourseSpec`。

验证：

- [ ] “三年级面积太空风格”能补出年级、学科、主题、候选目标、风格。
- [ ] 缺少年级时必须追问。
- [ ] 家长禁用上传图片时，参考图字段被拒绝或降级。
...
- [ ] `packages/core/src/tools/generate-style-preview.ts`
- [ ] `agent-test/docs/product/mvp2-style-preview.md`

实现要求：

- [ ] `StylePreview` 从 `StyleSpec` 和参考图描述生成，不直接进入完整素材生成。
- [ ] 方案确认展示 `CoursePlanOption` 的评分、风险、预计成本、预计时长、讲解深度差异。
- [ ] 课程深度调节后重新调用 1.0 的计划评分，不直接进入 GDD。

验证：

- [ ] 风格板输出必须包含 palette、characterDirection、uiMood、forbiddenElements。
- [ ] 深度从 `standard` 改为 `deep` 后，方案的成本和深度要求变化可见。
- [ ] 禁止知名 IP 风格直接进入 previewPrompt。
...
- [ ] `packages/core/src/course/product/preferenceProfile.ts`
- [ ] `packages/core/src/course/product/learningState.ts`
- [ ] `agent-test/docs/product/mvp2-learning-memory.md`

实现要求：

- [ ] 偏好记忆只影响风格、玩法倾向和交互偏好。
- [ ] 学习状态只影响课程目标、难度、错因和下一课推荐。
- [ ] 两类记忆必须分离，避免把学习弱点误当成兴趣偏好。
- [ ] 默认只保存结构化摘要，不保存完整对话和原始学生输入。

验证：

- [ ] 偏好更新不会修改学习状态。
- [ ] 学习报告中的错因能更新 `LearningState.misconceptionTags`。
- [ ] 数据删除调用能清除 profile 关联偏好和学习状态。
...
- [ ] `packages/core/src/course/product/courseRevision.ts`
- [ ] `packages/core/src/tools/revise-course-plan.ts`
- [ ] `packages/sdk-typescript/src/course/reviseCoursePlan.ts`

实现要求：

- [ ] 只允许编辑结构化计划：主题、深度、题目、角色/配色、视频开关、TTS 风格。
- [ ] 修改后重新校验 `CourseSpec`、`CoursePlanOption` 或 `CourseGDD`，不能直接改生成后源码。
- [ ] 修改题目时必须重新校验答案、解析、错因和提示。
- [ ] 关闭视频时自动更新 assetPlan 和 validationPlan。

验证：

- [ ] 改讲解深度后，缺少例题或迁移任务会失败。
- [ ] 替换题目但缺少 explanation 会失败。
- [ ] 禁用视频后 CourseGDD 中不能再有必需视频资产。
...
- [ ] `packages/core/src/tools/generate-next-course-spec.ts`
- [ ] `packages/sdk-typescript/src/course/createCourseGame.ts`

实现要求：

- [ ] 从 1.0 的 `LearningReport` 提取掌握点、错因、提示使用情况和下一步目标。
- [ ] 输出新的 `CourseSpec`，可以是下一节、强化练习或同主题新玩法。
- [ ] 下一课必须继承必要偏好，但不能盲目重复相同玩法。
- [ ] 如果学习状态不足，回退到输入向导追问。

验证：

- [ ] 错因“单位混淆”能生成单位换算或面积单位强化课程。
- [ ] 用户偏好太空风格能被继承，但允许新主题变化。
- [ ] 学习状态缺失时不生成课程，返回追问。
...
- [ ] `packages/core/src/course/product/guardianPolicy.ts`
- [ ] `packages/core/src/course/product/generationRecovery.ts`
- [ ] `agent-test/docs/product/mvp2-guardian-policy.md`

实现要求：

- [ ] 家长控制必须影响 CourseSpec 生成、资产计划和发布校验。
- [ ] 失败恢复记录当前阶段、失败原因、可复用产物、可降级选项。
- [ ] 支持从方案确认、CourseGDD、资产生成、TTS、验证失败处恢复。
- [ ] 重试必须有次数和成本上限。
