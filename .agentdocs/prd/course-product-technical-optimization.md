...
OpenGame 课程生成的核心价值不是“让 AI 从零写一个课程游戏”，而是在高质量工程底座上精准调度 AI，形成速度、稳定和全能三类优势。

### 1. 速度优势：模板优先，少从零生成

- 首选复用核心底座模板、课程 runtime、ready playlet、通用状态管理、验证协议和已有素材流程。
- AI 主要生成课程配置、玩法编排、关卡内容、反馈文案、素材 prompt 和风格导演信息，不应默认从空白 React/Vite 页面开始。
- 新课程的差异应主要来自学习目标映射、玩法组合、场景内容、视觉素材和反馈后果，而不是重复实现底层引擎。

### 2. 稳定优势：用工程化把 AI 不稳定变稳定

- 所有生成必须经过 `CourseSpec -> CoursePlanOption -> CourseGDD -> mapper -> course_runtime/playlet -> assets/TTS/video -> validate_course_package -> browser smoke` 的受控链路。
- AI 只能在明确 schema 和文件边界内发挥；玩法引擎、模板复制、状态流转、旁白降级、视频降级和验证由工程代码兜底。
- 工具错误、素材失败、TTS 失败、视频失败、构建失败都必须进入自动修复或降级协议，不能由最终回复口头兜底。

### 3. 全能优势：课程、玩法、教学目标深度映射

- 生成器必须先理解学习目标、学习阶段、知识形态和学生核心动作，再选择玩法；学科只提供素材语境。
- 课程质量来自“模板 + AI”的结合：模板保证交互手感和运行稳定，AI 负责把知识点变成合适的目标、角色、关卡、反馈、图片和视频表达。
- 图片和视频生成不是装饰清单，而是精准增强体验：图片用于角色、场景、关键道具、概念卡和反馈状态；视频用于开场/章节过场或关键状态变化，必须可选且可降级。

### 本次实现的顶级思想

- 安全改写和风格改写都要保留用户真实意图：危险、侵权或不适龄表象可以替换，但用户想要的身份、操作、情绪、节奏和挑战不能被删除。枪战可以改成水枪/泡沫飞镖/能量靶，侦探要保留搜证推理，建造要保留拼装验收，经营要保留资源权衡。
- Course Studio 不能允许模型绕过课程工具手写静态题目页；必须走模板、playlet、素材和验证链路。
- 质量门禁要检查游戏动作是否真的承载学习动作，尤其要拦截“用户核心诉求被当成表皮，结果生成连续做题或模板换皮”的退化。
- 模板是速度和稳定的来源，不是产品体验的最终形态。AI 必须在受控边界内创造独特角色、世界状态、奖励、错因反馈、素材 prompt 和过场构想，让用户感知到每门课都是围绕自己需求生成的。
